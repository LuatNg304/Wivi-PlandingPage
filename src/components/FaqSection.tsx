...
const faqs: FaqItem[] = [
  {
    question: "Phương pháp Hũ tài chính trong WIVI hoạt động như thế nào?",
    answer:
  ...
  {
    question:
      "Dữ liệu tài chính cá nhân của tôi có được bảo mật an toàn không?",
    answer:
  ...
  {
    question:
      "Tôi có thể dùng WIVI trên nhiều thiết bị (iPhone, Android, Máy tính) không?",
    answer:
      ...
      <div className="grid gap-10 md:grid-cols-5 items-start">
        ...
        <div className="md:col-span-2">
          <div className="max-w-xs">
            <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-900 md:text-5xl md:leading-tight">
              Câu hỏi <br />
            ...
            <p className="mt-2.5 sm:mt-3 text-xs sm:text-base text-slate-600 leading-relaxed">
              Những thắc mắc phổ biến nhất về cơ chế tự động hoá Hũ, AI OCR và
              tính an toàn bảo mật của WIVI.
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-3">
          <div className="divide-y divide-slate-200 rounded-xl sm:rounded-2xl border border-slate-200 bg-white px-4 sm:px-6 shadow-xs">
            {faqs.map((faq, index) => {
              ...
              return (
                <div key={index} className="py-3.5 sm:py-5">
                  <button
                    ...
                    className="flex w-full items-center justify-between text-start text-sm sm:text-lg font-bold text-slate-900 hover:text-blue-600 transition cursor-pointer gap-3 sm:gap-4"
                  >
                    <span className="leading-snug">{faq.question}</span>
                    <ChevronDown
                      className={`size-4 sm:size-5 shrink-0 text-blue-600 transition-transform duration-300 ${
                        isOpen ? "rotate-180" : ""
                      }`}
                    />
                  </button>
                  {isOpen && (
                    <div className="mt-2.5 sm:mt-3 text-xs sm:text-sm text-slate-600 leading-relaxed pr-2 sm:pr-6 animate-fadeIn">
                      <p>{faq.answer}</p>
                    </div>
                  )}
                </div>
